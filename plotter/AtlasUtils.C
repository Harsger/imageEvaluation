...
void myBoxText(Double_t x,Double_t y,Int_t mcolor,const char *text,Style_t fstyle=1001)
{
...
void my2MarkerText(Double_t x, Double_t y, Int_t color1, Int_t mstyle1, Int_t color2, Int_t mstyle2, const char *text, Float_t msize) 
{
  Double_t tsize=0.06;
  TMarker *marker = new TMarker(x-(0.8*tsize),y,8);
  marker->SetMarkerColor(color1);  marker->SetNDC();
  marker->SetMarkerStyle(mstyle1);
  marker->SetMarkerSize(msize);
  marker->Draw();

  marker = new TMarker(x-(0.4*tsize),y,8);
  marker->SetMarkerColor(color2);  marker->SetNDC();
  marker->SetMarkerStyle(mstyle2);
  marker->SetMarkerSize(msize);
  marker->Draw();
...
void my3MarkerText(Double_t x, Double_t y, Int_t color1, Int_t mstyle1, Int_t color2, Int_t mstyle2, Int_t color3, Int_t mstyle3, const char *text, Float_t msize) 
{
  Double_t tsize=0.06;
  TMarker *marker = new TMarker(x-(1.2*tsize),y,8);
  marker->SetMarkerColor(color1);  marker->SetNDC();
  marker->SetMarkerStyle(mstyle1);
  marker->SetMarkerSize(msize);
  marker->Draw();

  marker = new TMarker(x-(0.8*tsize),y,8);
  marker->SetMarkerColor(color2);  marker->SetNDC();
  marker->SetMarkerStyle(mstyle2);
  marker->SetMarkerSize(msize);
  marker->Draw();
  ...
  marker = new TMarker(x-(0.4*tsize),y,8);
  marker->SetMarkerColor(color3);  marker->SetNDC();
  marker->SetMarkerStyle(mstyle3);
  marker->SetMarkerSize(msize);
  marker->Draw();
...
void savePad(const char* id) {
  TVirtualPad* p=gPad;
  TString name = TString("plots/")+id;
  name += "_plot";
//   p->Print(name+".C");
//   p->Print(name+".png");
//   p->Print(name+".eps");
